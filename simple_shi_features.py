"""Feature pipeline imported from samkorostov/shield-model.
...
The implementation is kept separate from the noise-generation code so the
experimental SHI analysis can be reproduced without modifying collected data.
"""
...
def build_feature_pipeline(fs: float, include_ar: bool = False) -> FeatureUnion:
    transformers = [
...
def extract_feature_batch(windows: np.ndarray, fs: float, include_ar: bool = False) -> np.ndarray:
    """Extract the upstream feature set from a 2-D window batch.

    The default is the documented 22-feature pipeline. AR-Burg can be enabled
    explicitly, but remains off by default because the upstream README marks it
    disabled and its per-window solver dominates runtime on the long runs.
    """
...
def get_pipeline_feature_names(is_imu: bool = False, include_ar: bool = False) -> list[str]:
    base_names = list(
